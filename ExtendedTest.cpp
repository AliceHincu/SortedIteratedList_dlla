...
void testRemoveIterator(){
    cout << "Test function remove from iterator" << endl;
    SortedIteratedList list = SortedIteratedList(asc);
    for (int i = 0; i <= 20; i++){
        list.add(i);
    }

    ListIterator iterator = list.first();
    int count = 0;
    TComp elem;
    ...
    while (iterator.valid()) {
        // check for head
        if (count == 0){
            elem = iterator.remove();
            assert(elem == 0);
            assert(iterator.getCurrent() == 1); // *now tail should be at count 19
        }
        ...
        if (count == 10){
            elem = iterator.remove();
            assert(elem == 11);
            assert(iterator.getCurrent() == 12); // *now tail should be at count 18
        }
        ...
        if (count == 18){
            ...
            assert(elem == 20);
            try {
                assert(list.getElement(iterator));
                assert(false);
            } catch (exception&) {
                assert(true);
                break;
        ...
        //std::cout << count << ": " << iterator.getCurrent() << endl;
        count++;
        iterator.next();
    }
